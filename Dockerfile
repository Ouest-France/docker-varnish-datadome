...
RUN dnf install epel-release -y && dnf install varnish varnish-devel libtool automake libtool varnish-devel python3 make python3-docutils -y && dnf clean all
...
FROM centos:8
COPY ./asset/varnish.repo /etc/yum.repos.d/varnish.repo
RUN dnf install epel-release -y && dnf install varnish -y && dnf clean all
