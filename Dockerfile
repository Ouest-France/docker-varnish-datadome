...
RUN yum install epel-release -y && yum install varnish varnish-devel libtool python-docutils python3 make -y && yum clean all
RUN curl -L https://package.datadome.co/linux/DataDome-Varnish-6.3-latest.tgz -o ./DataDome-Varnish-6.3-latest.tgz &&tar -zxvf DataDome-Varnish-6.3-latest.tgz && cd DataDome-VarnishDome-* && \
